...
void STAR::iterate(std::function<void(std::shared_ptr<Runway>, std::shared_ptr<Fix>, std::shared_ptr<NavNode>)> f) const {
    // going to key (!), maps show where to come from
    std::multimap<std::shared_ptr<NavNode>, std::shared_ptr<NavNode>> routeToWaypoint;
    std::map<std::shared_ptr<NavNode>, std::shared_ptr<NavNode>> waypointToWaypoint;
    std::map<std::shared_ptr<Runway>, std::shared_ptr<NavNode>> runwayToApproachStart;
    std::map<std::shared_ptr<Runway>, std::shared_ptr<NavNode>> runwayToApproachEnd;

    auto accept = [&f] (std::shared_ptr<Runway> rw, std::shared_ptr<NavNode> start, std::shared_ptr<NavNode> end) {
    ...
    for (auto &it: runwayToApproachStart) {
        ...
        std::shared_ptr<NavNode> curFix = approachPoint;
