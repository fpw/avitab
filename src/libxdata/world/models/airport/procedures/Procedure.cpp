...
void Procedure::attachRunwayTransition(std::shared_ptr<Runway> rwy, const std::vector<std::shared_ptr<NavNode>> &nodes) {
    if (!rwy) {
...
void Procedure::attachCommonRoute(std::shared_ptr<NavNode> start, const std::vector<std::shared_ptr<NavNode>> &nodes) {
    commonRoutes.insert(std::make_pair(start, nodes));
}

void Procedure::attachEnrouteTransitions(const std::vector<std::shared_ptr<NavNode> >& nodes) {
