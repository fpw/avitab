...
void OverlayedMap::drawRoute() {
    if (!getRoute) {
        LOG_WARN("Function pointer unassigned - programming error");
        return;
    }
    auto route = getRoute();
    ...
    route->iterateLegs([this] (
        ...
        drawRouteLeg(fromLoc, toLoc, distanceNm, initialTrueBearing, initialMagneticBearing);
    });
}

void OverlayedMap::handleIDLcrossing(xdata::Location &to, xdata::Location &from,
    ...
    double toLongitude =
            (to.longitude < 0) ? to.longitude + 360 : to.longitude - 360;
    ...
    xdata::Location fromCross { latitudeCross, (from.longitude < 0) ? -180.0 : 180.0 };
    xdata::Location toCross { latitudeCross, (to.longitude < 0) ? -180.0 : 180.0 };
    double fromDistance = from.distanceTo(fromCross);
    double toDistance = toCross.distanceTo(to);
    drawRouteLeg(from, fromCross, fromDistance, trueBearing, magneticBearing);
    drawRouteLeg(toCross, to, toDistance, trueBearing, magneticBearing);
}

void OverlayedMap::drawRouteLeg(xdata::Location &from, xdata::Location &to, double distance,
    ...
    positionToPixel(from.latitude, from.longitude, fromX, fromY);
    positionToPixel(to.latitude, to.longitude, toX, toY);
...
std::shared_ptr<img::Image> OverlayedMap::createRouteAnnotation(int distance, int trueBearing, int magBearing) {
    const int font = 16;
    ...
    int textSize = std::max(font * 2 + 6, std::max(mapImage->getTextWidth(d, font), mapImage->getTextWidth(b, font))) + 2;
    int rotAngle = ((trueBearing + int(getNorthOffset()) + 180) % 180) - 90;
